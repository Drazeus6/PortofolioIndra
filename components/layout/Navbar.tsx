    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-dark-base/90 backdrop-blur-md border-b border-dark-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Desktop Links */}
          <nav className="hidden lg:flex items-center gap-1 bg-dark-surface p-1 rounded-md border border-dark-border font-mono">
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-xs font-semibold uppercase tracking-wider transition-all ${
                    active
                      ? isDev
                        ? 'bg-blue-600 text-white border border-blue-400 shadow-md shadow-blue-950/40'
                        : 'bg-amber-600 text-slate-950 border border-amber-400 shadow-md shadow-amber-950/40 font-bold'
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-md border border-dark-border bg-dark-card text-slate-300 hover:text-white transition-colors"
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-b border-dark-border bg-dark-surface px-4 pt-3 pb-6 space-y-2 font-mono text-xs">
          {navLinks.map((link) => {
            const active = pathname === link.href;
            const Icon = link.icon;
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-md uppercase font-bold tracking-wider transition-colors ${
                  active
                    ? isDev
